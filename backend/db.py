...
def get_db():
    """
    FastAPI-compatible dependency if you want to use Depends(get_db).
    For now, you can also manually create SessionLocal() inside endpoints.
    """
    db = SessionLocal()
    try:
        yield db
...
def get_or_create_patient(db, external_patient_id: str) -> Patient:
    """
    Map your P001 / P002 (string) to Patient row.
    If not present, create a minimal patient row (you can later update demographics).
    """
